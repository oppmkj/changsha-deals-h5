...
const fileMetas = {
    ...
    "src/components/AuthModal/AuthModal.tsx": { desc: "底部全局唤起的登录及验证交互弹窗", feat: ["分步式收集手机及安全短信验证码", "提供演示环境快速通过入口"], dep: "react, useAuth" }
}
...
for (const [relPath, meta] of Object.entries(fileMetas)) {
    ...
    // Remove existing top comment blocks until the first import or export or function
    // We can do this by looking at whatever is before the first import/export/const
    const codeStartIndex = content.search(/^(import|export|const|function|var|let)\b/m)

    if (codeStartIndex !== -1) {
        const header = content.slice(0, codeStartIndex)
        // If it's pure comments/whitespace, remove it
        if (/^[\s\n]*(\/\*[\s\S]*?\*\/|(\/\/.*\n)*)[\s\n]*$/.test(header)) {
            content = content.slice(codeStartIndex)
        }
    }

    // Construct standard comment block
 ...
 * @last_updated ${dateStr} - 根据全局代码规范要求补充标准头部说明注释
 */\n`

    fs.writeFileSync(fullPath, commentBlock + content)
    console.log(`Updated header for ${relPath}`)
}
